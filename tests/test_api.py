...
def test_upload_document_success() -> None:
    """Test successful upload, parsing, chunking, and db serialization to a strict workspace."""
    ws_id = _create_test_workspace("Upload Test Workspace")
...
def test_workspace_inventory_endpoint() -> None:
    ...
    client.post(
        "/upload/",
        files={"file": ("inventory_test.txt", file_content, "text/plain")},
        data={"workspace_id": ws_id},
    )

    response = client.get(f"/inventory/{ws_id}")
    assert response.status_code == 200
    data = response.json()
